 ...
 */
class TablaSesiones private constructor(private val sesionesDao: SesionesDao) {
    ...
    companion object {
        @Volatile
        private var INSTANCE: TablaSesiones? = null

        /**
         * Método para obtener la instancia única de TablaSesiones.
         *
         * @param sesionesDao DAO de la tabla sesiones
         * @return Instancia única de TablaSesiones
         */
        fun getInstance(sesionesDao: SesionesDao): TablaSesiones {
            return INSTANCE ?: synchronized(this) {
                val instance = TablaSesiones(sesionesDao).also { INSTANCE = it }
                Log.d("TablaSesiones", "🚀 Instancia de TablaSesiones creada: ${instance.hashCode()}")
                instance
            }
        }
    }
    ...
    suspend fun contar() {
        try {
            Log.d("TablaSesiones", "📌 Iniciando consulta SELECT COUNT(*) FROM sesiones")
            ...
            Log.d("TablaSesiones", "✅ Conteo de registros en la tabla 'sesiones': $total")
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaSesiones", "❌ Error al ejecutar la consulta SELECT COUNT(*)", e)
            throw e
        }
    }
    ...
    suspend fun obtenerTodos() {
        try {
            Log.d("TablaSesiones", "📌 Iniciando consulta SELECT * FROM sesiones")
            ...
            // Mostrar el resultado en el log
            Log.d("TablaSesiones", "✅ Registros obtenidos: ${registros}")
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaSesiones", "❌ Error al ejecutar la consulta SELECT *", e)
            throw e
        }
    }
    ...
    suspend fun insertarRegistro(registro: Sesiones) {
        try {
            Log.d("TablaSesiones", "📌 Insertando registro en la tabla 'sesiones'")
            ...
            Log.d("TablaSesiones", "✅ Registro insertado correctamente")
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaSesiones", "❌ Error al insertar registro", e)
            throw e
        }
    }
    ...
    suspend fun eliminarRegistro(id: Int) {
        try {
            Log.d("TablaSesiones", "📌 Eliminando registro con ID: $id")
            ...
            // Mostrar confirmación en el log
            Log.d("TablaSesiones", "✅ Registro eliminado correctamente")
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaSesiones", "❌ Error al eliminar registro", e)
            throw e
        }
    }
    ...
    suspend fun vaciarTabla() {
        try {
            Log.d("TablaSesiones", "📌 Vaciando tabla 'sesiones'")
            ...
            // Mostrar confirmación en el log
            Log.d("TablaSesiones", "✅ Tabla vaciada correctamente")
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaSesiones", "❌ Error al vaciar tabla", e)
            throw e
        }
    }
    ...
    suspend fun agregaRetorno(idBd: Int) {
        try {
            Log.d("TablaSesiones", "📌 Agregando nuevo registro en la tabla 'sesiones'")
            ...
            // Mostrar confirmación en el log
            Log.d("TablaSesiones", "✅ Nuevo registro agregado con ID: $idGenerado")
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaSesiones", "❌ Error al agregar registro", e)
            throw e
        }
    }
    ...
    suspend fun cerrarSesion(idSesion: Int) {
        try {
            Log.d("TablaSesiones", "📌 Cerrando sesión con ID: $idSesion")
            ...
            if (sesion != null) {
                ...
                // Mostrar confirmación en el log
                Log.d("TablaSesiones", "✅ Sesión cerrada correctamente")
            } else {
                Log.e("TablaSesiones", "❌ Sesión no encontrada con ID: $idSesion")
                throw IllegalArgumentException("Sesión no encontrada con ID: $idSesion")
            }
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaSesiones", "❌ Error al cerrar sesión", e)
            throw e
        }
    }
    ...
    suspend fun mostrarRegistros() {
        try {
            Log.d("TablaSesiones", "📌 Iniciando consulta SELECT * FROM sesiones ORDER BY id")
            ...
            if (registros.isNotEmpty()) {
                Log.d("TablaSesiones", "✅ Registros obtenidos (ordenados por ID):")
                for (registro in registros) {
                    Log.d("TablaSesiones", "   - ID: ${registro.id}, BD: ${registro.bd}, Fecha: ${registro.fecha}, Cierre: ${registro.cierre}")
                }
            } else {
                Log.d("TablaSesiones", "✅ No hay registros en la tabla 'sesiones'.")
            }
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaSesiones", "❌ Error al ejecutar la consulta SELECT * ORDER BY id", e)
            throw e
        }
    }
}
