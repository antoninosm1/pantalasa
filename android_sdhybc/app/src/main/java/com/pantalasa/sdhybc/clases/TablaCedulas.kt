...
import com.pantalasa.sdhybc.bd.tablas.ComboItem
 ...
 */
class TablaCedulas private constructor(private val cedulaDao: CedulaDao) {
    ...
    init {
        // Inicializar la lista de campos con valores predeterminados
        inicializarCampos()
    }

    companion object {
        @Volatile
        private var INSTANCE: TablaCedulas? = null

        /**
         * Método para obtener la instancia única de TablaCedulas.
         *
         * @param cedulaDao DAO de la tabla cedulas
         * @return Instancia única de TablaCedulas
         */
        fun getInstance(cedulaDao: CedulaDao): TablaCedulas {
            return INSTANCE ?: synchronized(this) {
                val instance = TablaCedulas(cedulaDao).also { INSTANCE = it }
                Log.d("TablaCedulas", "🚀 Instancia de TablaCedulas creada: ${instance.hashCode()}")
                instance
            }
        }
    }
    ...
    fun mostrarEstructura() {
        Log.d("TablaCedulas", "📌 Mostrando estructura de campos:")
        for (campo in campos) {
            Log.d("TablaCedulas", "   - Nombre: ${campo.nombreCampo}, Tipo: ${campo.tipoCampo}, Concatenación: ${campo.fuenteConcatenacion}, Valores: ${campo.fuenteValores}")
        }
    }
    ...
    suspend fun contar() {
        try {
            Log.d("TablaCedulas", "📌 Iniciando consulta SELECT COUNT(*) FROM cedulas")

            // Ejecutar la consulta y obtener el conteo
            total = cedulaDao.obtenerConteo()

            // Mostrar el resultado en el log
            Log.d("TablaCedulas", "✅ Conteo de registros en la tabla 'cedulas': $total")
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaCedulas", "❌ Error al ejecutar la consulta SELECT COUNT(*)", e)
        }
    }
    ...
    suspend fun obtenerTodos() {
        try {
            Log.d("TablaCedulas", "📌 Iniciando consulta SELECT * FROM cedulas")

            // Ejecutar la consulta y obtener los registros
            val registros = cedulaDao.obtenerCedulas()

            // Mostrar el resultado en el log
            Log.d("TablaCedulas", "✅ Registros obtenidos: ${registros}")
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaCedulas", "❌ Error al ejecutar la consulta SELECT *", e)
        }
    }
    ...
    suspend fun insertarRegistro(registro: Cedula) {
        try {
            Log.d("TablaCedulas", "📌 Insertando registro en la tabla 'cedulas'")

            // Insertar el registro
            cedulaDao.insertar(registro)

            // Mostrar confirmación en el log
            Log.d("TablaCedulas", "✅ Registro insertado correctamente")
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaCedulas", "❌ Error al insertar registro", e)
        }
    }
    ...
    suspend fun eliminarRegistro(id: Int) {
        try {
            Log.d("TablaCedulas", "📌 Eliminando registro con ID: $id")

            // Eliminar el registro
            cedulaDao.eliminar(id.toLong())

            // Mostrar confirmación en el log
            Log.d("TablaCedulas", "✅ Registro eliminado correctamente")
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaCedulas", "❌ Error al eliminar registro", e)
        }
    }
    ...
    suspend fun vaciarTabla() {
        try {
            Log.d("TablaCedulas", "📌 Vaciando tabla 'cedulas'")
            ...
            // Mostrar confirmación en el log
            Log.d("TablaCedulas", "✅ Tabla vaciada y contador de ID reiniciado correctamente")
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaCedulas", "❌ Error al vaciar tabla", e)
        }
    }
    ...
    suspend fun borrarWeb() {
        try {
            Log.d("TablaCedulas", "📌 Eliminando registros con ambito = 1")

            // Ejecutar la consulta para eliminar registros
            cedulaDao.eliminarPorAmbito(1)

            // Mostrar confirmación en el log
            Log.d("TablaCedulas", "✅ Registros eliminados correctamente")
            this.statusFuente = false
        } catch (e: Exception) {
            // Capturar y mostrar cualquier error inesperado
            Log.e("TablaCedulas", "❌ Error al eliminar registros", e)
        }
    }
    ...
    suspend fun volcarRegistros(jsonSource: Any) {
        try {
            Log.d("TablaCedulas", "📌 Iniciando proceso de volcado de registros")
            ...
            if (tipoFuenteDetectado != tipoFuente) {
                Log.e("TablaCedulas", "❌ Tipo de fuente no coincide. Esperado: $tipoFuente, Detectado: $tipoFuenteDetectado")
                return
            }

            // Validar la estructura de campos antes de continuar
            if (!validarEstructuraCampos()) {
                Log.e("TablaCedulas", "❌ Estructura de campos no válida. No se pueden volcar registros.")
                return
            }
            ...
            when (tipoFuente) {
                1 -> {
                    if (resumenFuente) {
                        Log.w("TablaCedulas", "⚠️ resumenFuente=true no tiene efecto con tipoFuente=1")
                    }
            ...
            Log.d("TablaCedulas", "✅ Proceso de volcado de registros completado")
        } catch (e: Exception) {
            Log.e("TablaCedulas", "❌ Error inesperado durante el volcado de registros", e)
            throw e
        }
    }
    private suspend fun validarEstructuraCampos(): Boolean {
        try {
            Log.d("TablaCedulas", "📌 Validando estructura de campos")
            ...
            if (cantidadCamposEstructura != cantidadCamposEntidad) {
                Log.e("TablaCedulas", "❌ Número de campos no coincide: CamposEstructura ($cantidadCamposEstructura) != Cedula ($cantidadCamposEntidad)")
                return false
            }
            ...
            for (i in campos.indices) {
                ...
                if (nombreCampoEstructura != nombreCampoEntidad) {
                    Log.e("TablaCedulas", "❌ Nombre de campo no coincide en la posición $i: $nombreCampoEstructura != $nombreCampoEntidad")
                    return false
                }
            }

            Log.d("TablaCedulas", "✅ Estructura de campos válida")
            return true
        } catch (e: Exception) {
            Log.e("TablaCedulas", "❌ Error al validar la estructura de campos", e)
            return false
        }
    }
    ...
    private suspend fun procesarObjetoJSON(json: JSONObject) {
        Log.d("TablaCedulas", "📌 Procesando objeto JSON")
            ...
            for (i in startIndex until jsonArray.length()) {
                ...
                if (!(resumenFuente && i == startIndex && json.has("registros"))) {
                    ...
                    if (registrosInsertados % 1000 == 0) {
                        Log.d("TablaCedulas", "📌 Registros insertados: $registrosInsertados")
                    }
                } else {
                    Log.d("TablaCedulas", "📌 Objeto de resumen ignorado en posición $i")
                }
            }

            Log.d("TablaCedulas", "✅ Total de registros insertados: $registrosInsertados")
        } catch (e: Exception) {
            Log.e("TablaCedulas", "❌ Error al procesar arreglo JSON", e)
            throw e
        }
    }
    /**
    ...
    private suspend fun procesarArregloDeArreglosJSON(jsonArray: JSONArray) {
        Log.d("TablaCedulas", "📌 [ProcesarArregloDeArreglos] Iniciando procesamiento de JSON tipo 3")

        try {
            // 1. Validación inicial de estructura
            if (jsonArray.length() == 0) {
                Log.d("TablaCedulas", "⚠️ [ProcesarArregloDeArreglos] JSONArray principal vacío")
                return
            }

            // 2. Detección automática de resumen si no fue establecido
            if (resumenFuente) {
                Log.d("TablaCedulas", "✅ [ProcesarArregloDeArreglos] Modo con resumen activado")
            }
            ...
            for (i in 0 until jsonArray.length()) {
                ...
                Log.d("TablaCedulas", "📦 [ProcesarArregloDeArreglos] Procesando arreglo interno #$i con ${innerArray.length()} elementos")

                // 5. Validar que el arreglo interno tenga elementos
                if (innerArray.length() == 0) {
                    Log.d("TablaCedulas", "⚠️ [ProcesarArregloDeArreglos] Arreglo interno #$i vacío")
                    continue
                }
                ...
                val startIndex = if (resumenFuente) {
                    ...
                    if (posibleResumen?.has("registros") == true) {
                        ...
                        Log.d("TablaCedulas", "📌 [ProcesarArregloDeArreglos] Ignorando objeto de resumen en arreglo #$i")
                        1
                    } else {
                        Log.d("TablaCedulas", "⚠️ [ProcesarArregloDeArreglos] Primer elemento no es resumen en arreglo #$i")
                        0
                    }
                } else {
                    0
                ...
                for (j in startIndex until innerArray.length()) {
                    try {
                        ...
                        if (totalRegistrosInsertados % 1000 == 0) {
                            Log.d("TablaCedulas", "📊 [ProcesarArregloDeArreglos] Progreso: $totalRegistrosInsertados registros insertados")
                        }
                    } catch (e: Exception) {
                        Log.e("TablaCedulas", "❌ [ProcesarArregloDeArreglos] Error en arreglo #$i, posición $j: ${e.message}")
                    }
                }
            }

            // 10. Reporte final del procesamiento
            Log.d("TablaCedulas", "✅ [ProcesarArregloDeArreglos] Procesamiento completado: " +
            ...
            Log.e("TablaCedulas", "❌ [ProcesarArregloDeArreglos] Error inesperado en procesamiento", e)
            throw e
        }
    }
    /**
    ...
    private suspend fun insertarRegistroDesdeJSON(json: JSONObject) {
        //Log.d("TablaCedulas", "📌 [InsertarRegistro] Iniciando inserción desde JSON")
        ...
        try {
            ...
            for (campo in campos) {
                try {
                    //Log.v("TablaCedulas", "🔄 Procesando campo: ${campo.nombreCampo}")
                    ...
                    when (tipoFuenteValor as Int) {
                        ...
                        1 -> {
                            //Log.d("TablaCedulas", "⚡ Ignorando campo autoincremental: ${campo.nombreCampo}")
                            continue
                        }
                        ...
                        2 -> {
                            ...
                            //Log.d("TablaCedulas", "🔹 Valor directo para ${campo.nombreCampo}: ${valores[campo.nombreCampo]}")
                        }
                        ...
                        3 -> {
                                ...
                                4 -> {  // Fecha (tratada como Long timestamp)
                                    ...
                                    if (fechaStr.isNotEmpty()) {
                                        try {
                                            ...
                                            Log.e("TablaCedulas", "❌ Error al parsear fecha '$fechaStr' para ${campo.nombreCampo}")
                                            0L
                                        }
                                    } else {
                                        0L
                            ...
                            if (valor != null) {
                                ...
                                //Log.d("TablaCedulas", "📥 Valor desde JSON para ${campo.nombreCampo}: $valor")
                            } else {
                    ...
                    Log.e("TablaCedulas", "❌ Error procesando campo ${campo.nombreCampo}: ${e.message}")
                    throw e
                }
            }
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    Log.e("TablaCedulas", "❌ Error al construir/insertar registro", e)
                    throw e
                }
            }
        } catch (e: Exception) {
            Log.e("TablaCedulas", "❌ Error inesperado en insertarRegistroDesdeJSON", e)
            throw e
        }
    }
    /**
    ...
    suspend fun muestra(intervalo: Int) {
        try {
            Log.d("TablaCedulas", "📌 Iniciando proceso de muestra de registros")

            // Obtener el conteo de registros
            total = cedulaDao.obtenerConteo()
            Log.d("TablaCedulas", "✅ Total de registros en la tabla 'cedulas': $total")

            // Mostrar el primer registro
            val primerRegistro = cedulaDao.obtenerPrimerRegistro()
            if (primerRegistro != null) {
                Log.d("TablaCedulas", "📌 Primer registro: $primerRegistro")
            } else {
                Log.d("TablaCedulas", "📌 No hay registros en la tabla.")
                return
            }
            ...
            for (i in intervalo until total step intervalo) {
                ...
                if (registro != null) {
                    Log.d("TablaCedulas", "📌 Registro en intervalo $i: $registro")
                }
            }
            ...
            if (ultimoRegistro != null) {
                Log.d("TablaCedulas", "📌 Último registro: $ultimoRegistro")
            }

            Log.d("TablaCedulas", "✅ Proceso de muestra de registros completado")
        } catch (e: Exception) {
            Log.e("TablaCedulas", "❌ Error inesperado durante la muestra de registros", e)
        }
    }

// En la clase TablaCedulas, modificar el método:
    ...
    suspend fun consultarLocalidadesCedulasCombo(): List<ComboItem> {
        return try {
            Log.d("TablaCedulas", "🔍 Consultando localidades para ComboList")
            val items = cedulaDao.consultarLocalidades()

            Log.d("TablaCedulas", "✅ ${items.size} localidades obtenidas")
            resultadoCombo = items
            items
        } catch (e: Exception) {
            Log.e("TablaCedulas", "❌ Error en consulta: ${e.message}")
            emptyList()
        }
    }
    ...
    fun inspeccionarResultadoCombo() {
        try {
            Log.d("TablaCedulas", "##%%## INSPECCIÓN resultadoCombo INICIADA")

            when (resultadoCombo.size) {
                0 -> Log.w("TablaCedulas", "⚠️ resultadoCombo VACÍO")
                1 -> Log.d("TablaCedulas", "📊 Único item: value='${resultadoCombo[0].value}' text='${resultadoCombo[0].text}'")
                else -> {
                    Log.d("TablaCedulas", "📊 Total items: ${resultadoCombo.size}")
                    Log.d("TablaCedulas", "   - Primer item: value='${resultadoCombo[0].value}'")
                    Log.d("TablaCedulas", "   - Último item: value='${resultadoCombo.last().value}'")
                }
            }
        } catch (e: Exception) {
            Log.e("TablaCedulas", "💥 Error en inspeccionarResultadoCombo()", e)
        } finally {
            Log.d("TablaCedulas", "##%%## INSPECCIÓN FINALIZADA")
        }
    }
}