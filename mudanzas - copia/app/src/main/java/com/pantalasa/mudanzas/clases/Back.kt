...
///////////////////////////////////////////////////////////////
// BLOQUE DE VERIFICACIÓN DE CONEXIÓN - Back.kt
// Ubicado dentro de la clase Back

    /**
     * Verifica el estado de conexión a internet y actualiza la variable statusConexion
     * Cumple con los principios:
     * - No recibe parámetros
     * - No retorna valores
     * - Actualiza solo la variable interna statusConexion
     * - Separa claramente las tareas
     */
    /**
     * Verifica el estado de conexión a internet y actualiza la variable statusConexion
     * con manejo robusto de errores y logging.
     */
    suspend fun verificarConexion() {
        ///////////////////////////////////////////////////////////////
        // BLOQUE 1: VERIFICACIÓN DE CONECTIVIDAD EN SEGUNDO PLANO CON MANEJO DE ERRORES
        val resultado = withContext(Dispatchers.IO) {
            try {
                ///////////////////////////////////////////////////////////////
                // BLOQUE 1.1: OBTENER SERVICIO DE CONECTIVIDAD CON PERMISOS
                val cm = contexto.getSystemService(Context.CONNECTIVITY_SERVICE) as? ConnectivityManager
                if (cm == null) {
                    Print.getInstance().print(mutableListOf(mutableListOf(7, "Error: ConnectivityManager no disponible", true)))
                    return@withContext false
                }

                ///////////////////////////////////////////////////////////////
                // BLOQUE 1.2: LÓGICA PARA ANDROID 10+ (API 29+)
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                    val network = cm.activeNetwork
                    if (network == null) {
                        Print.getInstance().print(mutableListOf(mutableListOf(3, "No hay red activa (Android 10+)", true)))
                        return@withContext false
                    }

                    val capabilities = cm.getNetworkCapabilities(network)
                    if (capabilities == null) {
                        Print.getInstance().print(mutableListOf(mutableListOf(3, "No capabilities para red activa", true)))
                        return@withContext false
                    }

                    // Verifica capacidades esenciales de conexión de manera más flexible
                    ...
                    Print.getInstance().print(mutableListOf(mutableListOf(3, "Capabilities - Internet: $tieneInternet, Validado: $validado", true)))
                    return@withContext tieneInternet // Prioriza tener acceso a internet sobre validación estricta
                }
                ///////////////////////////////////////////////////////////////
                // BLOQUE 1.3: LÓGICA PARA VERSIONES ANTERIORES
                else {
                    @Suppress("DEPRECATION")
                    val activeNetwork = cm.activeNetworkInfo
                    val estaConectado = activeNetwork?.isConnected == true
                    Print.getInstance().print(mutableListOf(mutableListOf(3, "Conexión legacy: $estaConectado", true)))
                    return@withContext estaConectado
                }
            } catch (e: Exception) {
                ///////////////////////////////////////////////////////////////
                // BLOQUE 1.4: MANEJO DE ERRORES CON LOGGING
                Print.getInstance().print(mutableListOf(mutableListOf(8, "Excepción en verificarConexion: ${e.message}", true)))
                return@withContext false
            }
        }

        ///////////////////////////////////////////////////////////////
        // BLOQUE 2: ACTUALIZACIÓN EXPLÍCITA DE ESTADO
        actualizarEstadoConexion(resultado)
    }
    /**
     * Actualiza el estado de conexión de forma explícita
     * Cumple con SEPARACIÓN DE TAREAS
     */
    private fun actualizarEstadoConexion(estado: Boolean) {
        ///////////////////////////////////////////////////////////////
        // BLOQUE 2.1: ASIGNACIÓN DIRECTA DE VARIABLE
        statusConexion = estado

        ///////////////////////////////////////////////////////////////
        // BLOQUE 2.2: REGISTRO DE LOG
        registrarEstadoConexion(estado)
    }

    /**
     * Registra el estado de conexión en logs
     * Cumple con SEPARACIÓN DE TAREAS
     */
    private fun registrarEstadoConexion(estado: Boolean) {
        ///////////////////////////////////////////////////////////////
        // BLOQUE 2.2.1: PREPARACIÓN DE MENSAJE DE LOG
        Print.getInstance().print(
            mutableListOf(
                mutableListOf(
                    3,  // Nivel de log
                    "Estado conexión actualizado: $estado",  // Mensaje
